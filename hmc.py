...
def stantardHMC(theta_0, eps, L, N, M=None, U=None, pi=None, visited=None):
    ...
    '''if U is None and pi is None and H is None:
        raise ValueError("U or pi or H must be given")
    if U is None and pi is not None:
        U = lambda x: -np.log(pi(x))
    gradU = grad(U)
    if H is None:
        H = lambda x: U(x[0]) + 0.5 * np.sum(x[1] * x[1])  # x[0]: theta, x[1]: v'''
